...
class TargetWalkStrategy(Strategy):
    ...
    def next_moves(self, think_time):
        ...
        while time.time() - t0 < timeout:
            ...
            score = self.target_walk(new_board, 'them', max_step)
            scoring[random_turn_index]['n_walks'] += 1
            scoring[random_turn_index]['sum'] += score
            count += 1

        print(f"\nI did {count} walks, among {len(all_possible_turns)} first turns")
    ...
    def target_walk(self, board, next_species, step_left):
        if step_left == 0:
            return self.heuristic(engine, board)
        else:
