...
class Engine():
    ...
    def get_possible_turns(self, board, creature_name):
        ...
        for c in combinations:
            only_one_move = list(
                filter(lambda x: len(x) == 1, c))  # combination of moves where the group is not divided
            nothing_changes = list(filter(lambda x: x[0].coord_init == x[0].coord_arriv,
                                          only_one_move))  # combination of moves where origin = destination for each : invalid
